...
function showUsageAndExit() {
    Write-Host "`r`n USAGE: deploy-mongo.ps1 <node-count> <dns-prefix> <image-name> <password> <location> <pub-settings-file-path> [replica-set-name]`r`n" -foregroundcolor "green"
    Write-Host " node-count             : Number of nodes in the replica-set" -foregroundcolor "green"
    ...
    Write-Host " replica-set-name       : The name of the replica-set [Optional]" -foregroundcolor "green"
    exit 1
}
...
if ($args.Count -eq 6) {
    ...
    $REPLICASETNAME = $args[6]
} else {
    logErr "Incorrect usage, missing required arguments"
    showUsageAndExit
}
...
{
    logErr "The <node-count> argument should be number greater than 0"
    showUsageAndExit
}

if (!(Test-Path -LiteralPath $args[5]  -PathType Leaf)) 
{
    logErr "The <pub-settings-file-path> argument should refer to a file containing publish settings"
    showUsageAndExit
}

if (($NODECOUNT -eq 1) -and ($args.Count -eq 7)) {
   logStatus "The [replica-set-name] argument will be ignored for single node MongoDB instance"
}

if (($NODECOUNT -gt 1) -and ($args.Count -eq 6)) {
   logStatus "The [replica-set-name] default to 'rs'"
}
...
<#
    ...
    @param string   $name        Name of the package
    @param string   $fName       The file name to use
...
#------------------ Import the publisher settings file and get the subscription ID-----------------------------------------
...
try
{
    ...
    $serHost = [regex]::replace([regex]::Replace($serHost, "^(?i)https://", ''), "/$", '')
...
# -----------------------------------------------Adding endpoint mapping for Mongo Service----------------------------------
$i = 3389;
$j = 27017;
$mongoPorts = $null
While ($i -lt (3389 + $NODECOUNT)) 
{
    ...
    $mongoPorts += ":"
    $VMNAME = $VMNAMEPREFIX + "-" + $i
    logStatus "azure.cmd vm endpoint create $VMNAME $j $j  -d $DNSPREFIX"
    & $nodejsPath $nodeIAASToolAzureJS vm endpoint create $VMNAME $j $j -d $DNSPREFIX
    If ($lastexitcode -ne 0)
    {
        logErr "Failed to add Mongo endpoint to VM"
        exit 1
    }
    
    logStatus "endpoint mapping [$j, $j] added for the VM '$VMNAME'"
    $i++;
    $j++;
}
...
logStatus "Initiating mongo installation on VMs"
.\remote-setup.ps1 $PASSWORD
